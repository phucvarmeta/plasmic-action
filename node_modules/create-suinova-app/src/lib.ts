...
export async function create(args: CreatePlasmicAppArgs): Promise<void> {
  ...
  console.log("1. Authenticate with SuiNova");
  console.log("2. Create a React/Next/Gatsby repo");
  console.log("3. Integrate with SuiNova");

  // Authenticate with Plasmic
  banner("AUTHENTICATING WITH SUINOVA");
  if (projectApiToken) {
    ...
    const promise = auth({
      ...
      .catch(() => {
        throw new Error(
          "Failed to authenticate with SuiNova. Please run `npx suinova-cli auth` manually."
        );
      });
    await promise;
  }
  ...
  // Install dependency
  banner("INSTALLING THE SUINOVA DEPENDENCY");
