...
export class PlasmicApi {
  ...
  async exportProject(
    ...
    const url = `${this.codegenHost}/api/v1/projects/${projectId}/code/components?branchName=${branchName}&export=true`;
  ...
  async projectMeta(projectId: string) {
    const url = `${this.codegenHost}/api/v1/projects/${projectId}/code/meta`;
    const result = await this.post(url);
  ...
  async genLocalizationStrings(
    ...
    const url = `${
      this.codegenHost
    }/api/v1/localization/gen-texts?${params.toString()}`;
    const result = await this.get(url, undefined, {
      "x-plasmic-api-project-tokens": projectIdsAndTokens
        .map(
          ({ projectId, projectApiToken }) => `${projectId}:${projectApiToken}`
        )
        .join(","),
    });
  ...
  async uploadBundle(
    ...
    const url = `${this.codegenHost}/api/v1/projects/${projectId}/jsbundle/upload`;
    const result = await this.post(url, {
      projectId,
      bundleName,
      bundleJs,
  ...
  async projectStyleTokens(
    ...
    const url = `${this.codegenHost}/api/v1/projects/${projectId}/code/tokens?branchName=${branchName}`;
    const result = await this.post(url, { versionRange });
    return result.data as StyleTokensMap;
  }
  ...
  async projectIcons(
    ...
    const url = `${this.codegenHost}/api/v1/projects/${projectId}/code/icons?branchName=${branchName}`;
    const result = await this.post(url, { versionRange, iconIds });
